...
and the org Project board **terrella 1.0**. Architectural decisions are recorded as ADRs in
[docs/adr/](docs/adr/).
...
2. **One validated config in, everything generated out** — user intent lives in
   `terrella.yaml`; quadlets, gateway config, dashboards, and timers are rendered artifacts
...
|---|---|---:|---|
| **M0 — Fedora migration** | Full stack on Fedora 44 via hand-written quadlets; data restored from the WSL install; `terrella.target` replaces the `wsl --shutdown` gaming toggle | M | 🔄 active |
| **M1 — terrella CLI MVP** | `terrella provision && terrella apply` reproduces M0 from `terrella.yaml`; golden-file tests against the M0 quadlets; full CI matrix | L | ⬜ |
| **M2 — Config, secrets & 2026 models** | Gateway + serving fully config-rendered; podman secrets + sops-age; 2026 model refresh for 16 GB VRAM; DB migrations + `terrella_*` views | M | ⬜ |
...
| **M4 — Agentic ops v1** | `terrella mcp serve` stack-ops MCP server; systemd-timer agents (bench regression, spend report, catalog watcher) | L | ⬜ |
...
| **M6 — OSS launch** | De-personalized docs content, GitHub Releases automation, public announcement. (Pulled forward to the terrella rename: LICENSE/CONTRIBUTING/SECURITY/CoC, REUSE, PyPI name + Trusted Publishing.) | S | ⬜ |
...
- `stack/` (compose) and `provision/` (bash) remain the working reference through M0 and are
  ...
  `terrella apply` reproduces them equivalently. **No new features land in the legacy scripts
  after M0.**
...
- ~~Check PyPI/name availability at M1 scaffold time; final naming and licensing decisions
  land at M6.~~ **Resolved early (July 2026):** the project renamed to **terrella**
  ([ADR-0008](docs/adr/ADR-0008-project-name-terrella.md)), the PyPI name is reserved, and
  licensing landed as Apache-2.0 ([ADR-0009](docs/adr/ADR-0009-licensing-and-compliance.md)).
