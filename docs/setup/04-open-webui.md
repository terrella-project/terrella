...
- **`OLLAMA_BASE_URL=http://127.0.0.1:11434`** — explicitly points it at the local ollama. Combined with `OLLAMA_HOST=0.0.0.0` from [ollama setup](03-ollama.md), it works on day one.
...
1. **Settings → Models**: Open WebUI auto-discovers ollama models via `OLLAMA_BASE_URL`. You should see all the models you pulled in [ollama setup](03-ollama.md).
...
> If you don't want cloud models inside Open WebUI (because you'd rather route them through LiteLLM for accounting), skip step 2. You can also point Open WebUI at LiteLLM as its OpenAI provider (`http://127.0.0.1:4000`) once the [full stack](06-observability-stack.md) is up.
...
Browser loads at <http://127.0.0.1:8080>, model picker shows your local models, and you can chat with `qwen2.5-coder:14b`. → on to [Aider setup](05-aider.md).
