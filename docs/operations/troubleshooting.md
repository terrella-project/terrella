...
3. **Backend keys missing** — confirm `ANTHROPIC_API_KEY` / `GEMINI_API_KEY` are in your shell when you run `docker compose up -d` (the env values are interpolated at compose-up time, **not** at container start).
