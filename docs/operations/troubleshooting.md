...
3. **Backend keys missing** — open `stack/.env` and confirm `ANTHROPIC_API_KEY` / `GEMINI_API_KEY` are filled in (not blank). Then `docker compose restart litellm` to reload the values.
