# ADR-0001 — earth-ai becomes an installable tool; CLI in Python

**Date:** 2026-07-01 · **Status:** Accepted

## Context

earth-ai started as a personal AI-workstation blueprint: documentation, bash provisioning,
...
earth-ai's end state is an **installable open-source tool/platform**: a CLI (working name
`earthai`) that provisions and manages a personal AI stack on any Linux box, config-driven
...
`uv tool install earthai` (with a curl-able `install.sh` bootstrap):
