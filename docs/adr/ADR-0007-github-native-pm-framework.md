...
tracking system. Earlier projects evolved a proven GitHub-native model, including documented
"Rev 2" lessons.

## Decision

Adopt that proven framework, tailored to earth-ai (full model:
[docs/project-management.md](../project-management.md)):

- **Repo transferred to an org** to gain org issue types
  (Epic/Feature/Task/Spike/Bug).
- **Issue types classify work** (no `type:*` labels); `component:*` labels
...
- **PM-as-code**: `.github/project.yml` reconciled non-destructively by
  `scripts/project-sync.sh` — the same config-in/artifacts-out
...
- Delivery loop: conventional commits with component scopes, PR-title lint, enforced
  CHANGELOG (`no-changelog` escape), PR-only flow. **No DCO** (DCO +
...
- Local clones and docs referencing the old repo slug update to the new org remote
